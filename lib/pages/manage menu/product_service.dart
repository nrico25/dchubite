...
class ProductService {
  ...
  static Future<Product> createProduct(String token, Product product, File? imageFile) async {
    var request = http.MultipartRequest("POST", Uri.parse('${ApiEndpoint.baseUrl}/products'));
    ...
    if (response.statusCode == 201 && body['status'] == true) {
      ...
      throw Exception("Gagal menambahkan produk: ${body['message'] ?? responseData}");
    }
  }

  static Future<Product> updateProduct(String token, int id, Product product, File? imageFile) async {
    var request = http.MultipartRequest("POST", Uri.parse('${ApiEndpoint.baseUrl}/products/$id'));
    request.headers["Authorization"] = "Bearer $token";
    request.headers["Accept"] = "application/json";
    ...
    if (response.statusCode == 200 && body['status'] == true) {
      ...
      throw Exception("Gagal memperbarui produk: ${body['message'] ?? responseData}");
    }
  }
